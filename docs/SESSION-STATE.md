...
- **CI/CD wired** (GitHub Actions): PR guards (anti-relock/conflict) + build + Test-scheme tests run green; the sign/notarize/appcast/semantic-release/gh-release tail is gated on a `release-gate` step and **auto-activates** when the owner adds `APPLE_*` secrets (so push CI is green now, not red on missing secrets). Guard B grep was fixed to match the shipped `guard false else { return .pro }` patch (it had checked for the PLAN's prescribed `if false`/`alt-tab-free [depaywall]` form, which was never what shipped). Commit headers must be ≤72 chars (repo commitlint). Full status: `docs/CICD-STATUS.md`.
